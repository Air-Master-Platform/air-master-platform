# Sky Vision Agent (n8n)

n8n workflow for the Sky Vision Airlines cargo intake agent. OpenAI-backed. Greets the user
and collects shipment **L/W/H (cm), Weight (kg), Quantity**, then confirms.

## Nodes
- **Chat Trigger** — hosted chat UI. Sends the welcome message:
  *"Hi, this is the Sky Vision Airlines cargo agent. Could you send me your shipment L/W/H, weight and quantity?"*
- **Sky Vision Agent** — LangChain AI Agent. System prompt drives the intake + confirm flow.
...
## Import
1. n8n → **Workflows → Import from File** → select `sky-vision-agent.json`.
